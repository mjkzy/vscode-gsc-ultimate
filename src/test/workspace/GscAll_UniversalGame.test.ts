...
suite('GscAll.UniversalGame', () => {
    ...
    test('GscAll.UniversalGame.ItselfInclude', async () => {
        try {
            ...
            tests.checkHover(hover1, GscMarkdownGenerator.generateFunctionDescription({name: "func1", parameters: []}, true, tests.filePathToUri("GscAll.UniversalGame/scripts/ItselfInclude.gsc").toString(), undefined, undefined).value);
